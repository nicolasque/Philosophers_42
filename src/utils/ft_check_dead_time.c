...
/*   Updated: 2025/03/18 12:28:10 by nquecedo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_check_dead_time(t_philo *t_philo)
{
    ...
    // Check if another philosopher is already dead
    ...
    // Only check for death if philosopher is still alive
    if (t_philo->live == ALIVE && time_since_last_meal >= t_philo->t_shared->time_to_die)
    {
        t_philo->live = DEAD;
        t_philo->t_shared->philos_live = DEAD; // Set global death flag
        is_dead = DEAD;
    }
    else {
        is_dead = ALIVE;
