...
/*   Updated: 2025/03/11 14:25:28 by nquecedo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_eat(t_philo *t_philo)
{
	...
	ft_check_dead_time(t_philo);
	if (t_philo->t_shared->philos_live == DEAD)
		return (pthread_mutex_unlock(&t_philo->t_shared->print_mutex), pthread_mutex_unlock(&t_philo->t_shared->forks[t_philo->left_fork]), pthread_mutex_unlock(&t_philo->t_shared->forks[t_philo->right_fork]), (void)0);
	printf("%s%lld %d has taken a fork%s\n", YELLOW,
		   (get_time_mls() - t_philo->t_shared->program_star_time),
		   t_philo->id_philo, RESET);
	pthread_mutex_lock(&t_philo->t_shared->print_mutex);
	if (t_philo->t_shared->philos_live == DEAD)
		return (pthread_mutex_unlock(&t_philo->t_shared->print_mutex),pthread_mutex_unlock(&t_philo->t_shared->forks[t_philo->left_fork]) ,pthread_mutex_unlock(&t_philo->t_shared->forks[t_philo->right_fork]), (void)0);
	printf("%s%lld %d has eaten%s\n", CYAN, (get_time_mls() - t_philo->t_shared->program_star_time), t_philo->id_philo, RESET);
	pthread_mutex_unlock(&t_philo->t_shared->print_mutex);
	t_philo->times_eaten++;
...
void ft_sleep(t_philo *t_philo)
{
	...
	printf("%s%lld %d is sleeping%s\n", PURPLE, (get_time_mls() - t_philo->t_shared->program_star_time), t_philo->id_philo, RESET);
	pthread_mutex_unlock(&t_philo->t_shared->print_mutex);
...
void ft_think(t_philo *t_philo)
{
	...
	if (t_philo->t_shared->philos_live == DEAD)
		return (pthread_mutex_unlock(&t_philo->t_shared->print_mutex), (void)0);
	printf("%s%lld %d is thinking%s\n", GREEN, (get_time_mls() - t_philo->t_shared->program_star_time), t_philo->id_philo, RESET);
	pthread_mutex_unlock(&t_philo->t_shared->print_mutex);
}
